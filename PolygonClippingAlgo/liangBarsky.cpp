...
void clipLine(dcPt winMin, dcPt winMax, wcPt2 p1, wcPt2 p2)
{
    ...
    if (clipTest(-dx, p1.x - winMin.x, &t1, &t2))
        ...
        {
            dy = p2.y - p1.y;
            if (clipTest(-dy, p1.y - winMin.y, &t1, &t2)) // Removed &amp;

                // inside test wrt top edge
                if (clipTest(dy, winMax.y - p1.y, &t1, &t2)) // Removed &amp;
                {
                    if (t2 < 1.0)
                    {
                        p2.x = p1.x + t2 * dx;
...
int main()
{
    ...
    wcPt2 p2 = {400.0, 300.0}; // End point
    ...
    // Clip the line to the window boundaries
